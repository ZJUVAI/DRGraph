...
class Lock {
public:
  Lock() : flag_(ATOMIC_FLAG_INIT) {}
  Lock(const Lock&) : flag_(ATOMIC_FLAG_INIT) {}
...
class BitSet {
public:
  BitSet(std::size_t size, bool value) : size_(size), words_((size + 63) / 64, value ? ~0ULL : 0ULL) {
    if (value && !words_.empty() && size % 64 != 0) words_.back() &= (1ULL << (size % 64)) - 1ULL;
  }

  bool test(std::size_t index) const { return (words_[index / 64] & (1ULL << (index % 64))) != 0; }
  void set(std::size_t index) { words_[index / 64] |= 1ULL << (index % 64); }
  void set() { std::fill(words_.begin(), words_.end(), ~0ULL); }
  void reset(std::size_t index) { words_[index / 64] &= ~(1ULL << (index % 64)); }
...
{
public:
  InitIndex(const Matrix<DataType> &features, const Distance<DataType> *d, const IndexParams &params) : features_(features),
                                                                                                        distance_(d),
                                                                                                        params_(params)
  {
  }
  ...
  void reportBuildProgress(size_t completed, size_t total)
  {
    if (params_.build_progress_callback != nullptr)
      params_.build_progress_callback(params_.build_progress_context, completed, total);
  }
  ...
  void getResults(std::vector<int> *knn_vec, int n_vertices)
  {
  ...
  void setSearchParams(int epochs, int init_num, int extend_to, int search_trees, int search_lv, int search_method)
  {
    ...
    if (extend_to > init_num)
      SP.extend_to = init_num;
    else
      SP.extend_to = extend_to;
  ...
  void nnExpansion_kgraph(size_t K, const DataType *qNow, std::vector<unsigned int> &pool, std::vector<Point> &results)
  {
    ...
    BitSet tbflag(base_n, false);
    BitSet newflag(base_n, false);
    ...
    for (int i = 0; i < Iter; i++)
    {
      ...
      while (k < L)
      {
        ...
        if (newflag.test(knn[k].id))
        {
          ...
          {
            if (tbflag.test(neighbor->row_id))
              continue;
    ...
    for (int seg = 0; seg < segIter; seg++)
    {
      ...
      while (iter++ < SP.search_epoches)
      {
        ...
        for (size_t j = 0; j < ids.size(); j++)
        {
          ...
          Candidates.insert(c);
          newflag.set(ids[j]);
          if (Candidates.size() > (unsigned int)SP.extend_to)
  ...
  void join()
  {
    ...
    size_t cc = 0;
#pragma omp parallel for default(shared) schedule(dynamic, 100) reduction(+ \
                                                                          : cc)
...
#pragma omp parallel for
  ...
  void refineGraph()
  {
    ...
    while (iter++ < params_.build_epoches)
    {
      ...
      update(l);
      reportBuildProgress(static_cast<size_t>(iter + 1), static_cast<size_t>(params_.build_epoches + 1));
      f = clock();
      std::cout << "[kNN Graph] "
                << "iteration " << iter << " time: " << (f - s) * 1.0 / CLOCKS_PER_SEC << " seconds" << std::endl;
    }
    //calculate_norm();
    std::cout << "[kNN Graph] "
...
*/
    ...
    for (unsigned i = 0; i < nhoods.size(); i++)
    {
      ...
      // for(int j = 0; j < params_.Check_K - params_.K; ++j) {
      //   gs[i][j] =
      // }
...
#pragma omp parallel for
  ...
  std::vector<unsigned> getGraphRow(unsigned row_id)
  {
    ...
    if (gs.size() > row_id)
    {
      for (unsigned i = 0; i < gs[row_id].size(); i++)
        row.push_back(gs[row_id][i]);
    }
    return row;
  }
  std::vector<unsigned> getGraphCandidateRow(unsigned row_id)
  {
    std::vector<unsigned> row;
    if (g.size() > row_id)
    {
      row.reserve(g[row_id].size());
      for (unsigned i = 0; i < g[row_id].size(); ++i)
        row.push_back(g[row_id][i].id);
    }
    return row;
  }
