...
public:
	...
	tuplestruct	*insertItem(const tuplestruct newData);			// heap-inserts newData, returns the address of it
	tuplestruct	popMaximum();							// removes and returns heap max, reheapifies
	tuplestruct	returnMaximum();						// returns the heap max; no change to heap
	void		printHeap();							// displays contents of the heap
	void		printHeapTop10();						// displays top 10 entries in the heap
	void		updateItem(tuplestruct *address, tuplestruct newData);   // updates the value of the tuple at address
	void		updateItem(tuplestruct *address, double newStored);// update only the stored value of tuple at address
	void		deleteItem(tuplestruct *address);				// remove an item from the heap
	int		returnArraysize();						// 
	int		returnHeaplimit();						// 
...
// Pop Maximum ------------------------------------------------------------------------
tuplestruct maxheap::popMaximum() {				// O(log k) time
	tuplestruct temp = returnMaximum();
	deleteItem(A[1].d);
	return temp;
}

// Return Maximum --------------------------------------------------------------------
tuplestruct maxheap::returnMaximum() {			// O(1) time
	tuplestruct temp;
	temp.m = A[1].d->m;					// 
	temp.i = A[1].d->i;					// 
	temp.j = A[1].d->j;					// 
...
tuplestruct* maxheap::insertItem(const tuplestruct newData) {			// O(log k) time
	int index;
	tuplestruct *pointer;
...
void maxheap::updateItem(tuplestruct *address, tuplestruct newData) {   // O(log k) time
...
void maxheap::updateItem(tuplestruct *address, double newStored) {	// O(log k) time
	double oldm    = address->m;
...
void maxheap::deleteItem(tuplestruct *address) {
	tuplestruct *swap;
