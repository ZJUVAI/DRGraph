...
public:
	...
	KDTreeUbIndex(const Matrix<DataType>& dataset, const Distance<DataType>* d, const IndexParams& params = KDTreeUbIndexParams(true,4)) :
		BaseClass(dataset,d,params), rootlock_initialized_(false)
	{
	...
	~KDTreeUbIndex() override {
		for (size_t i = 0; i < tree_roots_.size(); ++i) delete tree_roots_[i];
		if (rootlock_initialized_) omp_destroy_lock(&rootlock);
	}
	...
	void buildIndexImpl(){
		...
		s = clock();
#endif
		this->reportBuildProgress(0, static_cast<size_t>(this->params_.build_epoches + 1));
		initGraph();
		this->reportBuildProgress(1, static_cast<size_t>(this->params_.build_epoches + 1));
	...
	void getNeighbors_nnexp(size_t K, const Matrix<DataType>& query){
		...
		for(unsigned int cur = 0; cur < query.get_rows(); cur++){
			BitSet tbflag(features_.get_rows(), false);
			BitSet newflag(features_.get_rows(), true);
			tbflag.reset();
			newflag.set();
...
#pragma omp parallel for
		for(unsigned int cur = 0; cur < query.get_rows(); cur++){
			...
			BitSet flags(features_.get_rows(), false);
...
#pragma omp parallel for
		for(unsigned i = 0; i < (unsigned)TreeNum; i++){
			...
			std::mt19937 shuffle_rng(seed ^ i);
			std::shuffle(myids.begin(), myids.end(), shuffle_rng);
		}
		if (!rootlock_initialized_) {
			omp_init_lock(&rootlock);
			rootlock_initialized_ = true;
		}
