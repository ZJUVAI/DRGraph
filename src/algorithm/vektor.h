...
public:
	...
	tuplestruct	*heap_ptr;			// pointer to element's location in vektor max-heap
...
public:
	...
	tuplestruct	returnMaxKey();							// returns the maximum key in the tree
	tuplestruct	returnMaxStored();							// returns a tuple of the maximum (key, .stored)
...
tuplestruct vektor::returnMaxStored() { return heap->returnMaximum(); }

tuplestruct vektor::returnMaxKey() {
	tuplestruct themax;
	element *current;
	current = root;
...
void vektor::deleteItem(int killKey) {
	...
	if (z != NULL) {
		tuplestruct newmax    = heap->returnMaximum();		// get old maximum in O(1)
...
void vektor::printTree() {
	tuplestruct max = heap->returnMaximum();
